...
os.chdir("C:/Chenyuan/2023秋季学期/SDA/期末/datamentioned") 
...
for file in os.listdir(root_dir):
    if file.endswith('.txt'):
        ...
        data.append({'Year': year, 'Content': cleaned_content})
...
stopWordsFilePathdata = r'C:/Chenyuan/2023秋季学期/SDA/期末/数据提取_停用词.txt'
with open(stopWordsFilePathdata, 'r', encoding='utf-8') as file:   
    stopWordsdata = file.readlines()
    stopWordsdata = [I.strip() for I in stopWordsdata]

highfreq = highfreq + stopWordsdata
...
dictionary.save(os.path.join('dictionaryd.dict')) 

'''
vocFile = open(os.path.join( 'vocabularyd.dat'),'w')
...
multFile = open(os.path.join( 'foo-multd.dat'),'w')
...
plt.legend(loc='best')
plt.title('Topic 1')
plt.savefig('Topic11.png',transparent=True)
plt.show()
